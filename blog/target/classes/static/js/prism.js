/* PrismJS 1.9.0
 http://prismjs.com/download.html?themes=prism&languages=markup+css+clike+javascript+c+cpp+java+json+python+jsx+sql */
        ...
        for (var c = 0; c < u.length; ++c) {
          ...
          for (var b = a, k = l; b < t.length; k += t[b].length, ++b) {
            ...
            if (!(w instanceof s)) {
              ...
              if (_) {
                d && (p = _[1].length);
                var A = _.index + p, _ = _[0].slice(p), j = A + _.length, S = w.slice(0, A), C = w.slice(j), M = [b, P];
